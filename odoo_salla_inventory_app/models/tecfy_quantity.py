...
class TecfySallaQuantity(models.Model):
    ...
    def prepareQuantities(self):
        ...
        for company in companies:
            ...
            sql = "select p.id from product_product p left join product_template t on t.id = p.product_tmpl_id where t.type = 'consu' and p.id not in (select product_id from tecfysalla_quantity where sync_id = {} ) and active = true limit {};".format(company.tecfy_salla_sync_id, count)
            ...
            for product in products:
                ...
                self.env.cr.commit()
            if len(productIds) < count: # if we finished all products

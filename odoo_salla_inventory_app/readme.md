...
for company in companies:
    ...
    sql = "select p.id, t.name, t.company_id, p.product_tmpl_id  from product_product p left join product_template t on t.id = p.product_tmpl_id where t.type = 'consu' and p.id not in (select product_id from tecfysalla_quantity where sync_id = {} ) limit {};".format(company.tecfy_salla_sync_id, count)
    env.cr.execute(sql)

...
class MultiChannelSale(models.Model):
    ...
    salla_verification_key = fields.Char(string="Verification Key", default=lambda self: self.get_verification_key())
    ...
    def get_redirect_url(self):
        return  urljoin(self.get_base_url(), 'salla/authenticate')
    ...
    def write(self, vals):
        if self.channel == "salla" and (vals.get('salla_client_id') or vals.get('salla_client_secret')):
    ...
    def connect_salla(self):
        ...
        # We can pass random value in this state value and verify with response for
        # two factor security, verification in controller (main.py)
        ...
        if response.status_code in [200, 201]:
            ...
            return self.display_message("<span class='text-danger'>Authentication failed, Please verify the added client id</p>")
    ...
    def import_salla(self, object, **kw):
        # if refresh token expired, channel state will be error
    ...
    def export_salla(self, record, **kw):
        # if token expired, channel will be in error
        self.with_context(operation=True).getAccessToken()
        with SallaApi(self.salla_client_id, self.salla_client_secret, self.access_token, self.refresh_token, channel=self, **kw) as api:
...
# ==================== Import Crons ==============================
    ...
    def salla_import_product_cron(self):
        _logger.info(
            "+++++ Import Product Cron is not supported in salla ecommerce ++++++")

    def salla_import_partner_cron(self):
        _logger.info(
            "+++++ Import Partner Cron is not supported in salla ecommerce ++++++")

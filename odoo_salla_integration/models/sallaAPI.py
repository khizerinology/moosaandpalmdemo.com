...
class SallaApi:
    ...
    def __init__(self, client_id, client_secret, access_token, refresh_token, channel=False, pricelist_id=False, **kw):
    ...
    def get_export_product_vals(self, record, options):
        if not (record.default_code or record.wk_default_code):
            ...
            elif record.wk_default_code:
                _logger.info('If there is no internal refernce or mulitple variants exists then parent code value will be used')
                sku = record.wk_default_code
        if sku:

...
class FetchData:
    ...
    def get_product_basic_vals(self, product):
        return {
            ...
            # 'image': self.channel_id.read_website_image_url(product.get('main_image')) if product.get('main_image') else False,
            'list_price': product.get("price").get('amount'),
            'weight': product.get("weight"),
            'wk_default_code': product.get("sku"),
            'default_code': product.get("sku"),
            'qty_available': product.get("quantity"),
            'extra_categ_ids': self.get_product_categories_string(product.get('categories'))
        }

    def get_product_categories_string(self, categories):
        category_ids = [str(x.get('id')) for x in categories]
        categ_string = ",".join(category_ids)
        category_data = []
        for category_id in category_ids:
            if not self.env['channel.category.mappings'].search([('channel_id', '=', self.channel_id .id), ('store_category_id', '=', category_id)]):
                kw = dict(
                    filter_type="id",
                    object_id=category_id,
                )
                vals, kw = self.channel_id.get_sallaApi().get_categories(**kw)
                category_data.extend(vals)
        if category_data:
            self.env['category.feed'].create(category_data)
        return categ_string
        ...
        try:
            ...
            if values:
                ...
                if not feed: #create product feed
                    ...
                    feed.write(vals)
                if self.channel_id.auto_evaluate_feed: #evaluate feed
                    feed.import_items()
